...
As mentioned earlier, we fit the model multiple times. At the end of week $w$, we fit the model to all the matches from week 1 to week $w$. In the code below, `epl_w` contains all the data for matches in the first $w$ weeks. We fit the model with 4 chains of length 800 (with the first half for warmup).
...
```{r, message=FALSE, warnings=FALSE, results=FALSE, echo=FALSE, comment=NA, fig.width=6, fig.height=6}
library ("arm")
library("rstan")
w <- 1
...
In the next figure, we plot the estimated abilities at the end of the season (i.e., after week 38). This time, we sort the teams in the tables according to their actual ranking at the end of the season. We observe that the estimated abilities are fairly consistent with the actual rankings and the uncertainty intervals are narrower compared to the results after week 1.

```{r, message=FALSE, warnings=FALSE, results=FALSE, echo=FALSE, comment=NA, fig.width=6, fig.height=5}
library ("arm")
library("rstan")
w <- 38
fit <- readRDS(paste("FITS/fit_", w, ".RDS", sep=""))
...
for (w in 1:38) { 
  ...
  sims <- extract(fit)
  a_sims <- sims$a[,w,]
  a_hat[w,] <- colMeans(a_sims)
  a_se[w,] <- sqrt(colVars(a_sims))
}
a_min <- a_hat-a_se 
a_max <- a_hat+a_se
